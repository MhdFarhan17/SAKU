...
import { ArrowLeft, Wallet, TrendingUp, Shield, CheckCircle2, AlertCircle } from 'lucide-react'
...
function UpdatePasswordForm() {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (res?.error) {
      setError(res.error)
      toast.error(t('auth_page.update_failed_title', 'Gagal Memperbarui'), { description: res.error })
    } else if (res?.success) {
      setSuccess(true)
      toast.success(t('auth_page.update_success_title', 'Password Diperbarui'), { description: t('auth_page.update_success_desc', 'Password Anda berhasil diperbarui. Silakan masuk.') })
      setTimeout(() => {
        router.push('/login')
      }, 2000)
    }
          ...
          >
            ...
            <div className="mb-8">
              <h2 className="text-[32px] md:text-[36px] font-black tracking-[-0.03em] text-[#0e0f0c] leading-tight">
                {t('auth_page.update_title', 'Atur Ulang Password')}
              </h2>
              <p className="text-[#868685] mt-2 leading-relaxed">
                {t('auth_page.update_desc', 'Silakan masukkan password baru Anda.')}
              </p>
            </div>
                ...
                {success ? (
                  ...
                  >
                    <div className="w-20 h-20 bg-[#e2f6d5] rounded-full flex items-center justify-center mb-2 border-4 border-[#ffffff] shadow-[0_0_0_1px_rgba(159,232,112,0.3)]">
                      <CheckCircle2 className="w-10 h-10 text-[#054d28]" />
                    </div>
                    <div>
                      <h3 className="text-xl font-black text-[#0e0f0c] mb-2">{t('auth_page.update_success_title', 'Password Diperbarui')}</h3>
                      <p className="text-[#868685] text-sm leading-relaxed">{t('auth_page.update_success_desc', 'Password Anda berhasil diperbarui. Silakan masuk.')}</p>
                    </div>
                  ...
                  >
                    ...
                    <div className="space-y-2">
                      <label className="text-xs font-black text-[#454745] uppercase tracking-wider">{t('auth_page.update_password_label', 'Password Baru')}</label>
                      <Input 
                        type="password" 
                        name="password" 
                        required 
                        minLength={6}
                        className="h-14 px-4 rounded-[16px] bg-[#fbfcfb] border-[#e2e6eb] text-[#0e0f0c] focus-visible:ring-2 focus-visible:ring-[#9fe870]/50 focus-visible:border-[#9fe870] shadow-none transition-all duration-300 text-[15px]"
                      />
                    </div>
                    ...
                    >
                      {loading ? (
                        <span className="flex items-center gap-2">
                          ...
                          {t('auth_page.update_loading', 'Memperbarui...')}
                        </span>
                      ) : t('auth_page.update_btn', 'Perbarui Password')}
                    </Button>
                  </motion.form>
                )}
              </AnimatePresence>
